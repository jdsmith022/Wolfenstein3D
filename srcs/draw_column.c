...
/*   Updated: 2020/03/13 16:55:18 by mminkjan      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void			draw_column(t_wolf *wolf, t_project plane, int x, size_t texdex)
{
	...
	while (y < HEIGHT)
	{
		...
		if (y >= plane.y_start && y <= plane.y_end)
		{
			...
			wall_index = ((int)wall_y * wolf->graphics.wall[texdex]->size_line) + (plane.offset * wolf->graphics.wall[texdex]->bits_ppixel / 8) * 4;
